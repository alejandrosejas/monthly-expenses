...
async function seedTestData() {
    ...
    for (const category of categories) {
        await testDb.run('INSERT INTO categories (id, name, color, is_default, created_at) VALUES (?, ?, ?, ?, ?)', category.id, category.name, category.color, category.is_default, category.created_at);
    }
    // Sample expenses
    const expenses = [
        {
    ...
    for (const expense of expenses) {
        await testDb.run(`INSERT INTO expenses 
       (id, date, amount, category, description, payment_method, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, expense.id, expense.date, expense.amount, expense.category, expense.description, expense.payment_method, expense.created_at, expense.updated_at);
    }
    // Sample budget
    const budget = {
        id: 'budget-1',
    ...
    // Insert budget
    await testDb.run(`INSERT INTO budgets
     (id, month, total_budget, category_budgets, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?)`, budget.id, budget.month, budget.total_budget, budget.category_budgets, budget.created_at, budget.updated_at);
}
