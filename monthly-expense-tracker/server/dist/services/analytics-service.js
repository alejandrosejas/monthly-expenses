...
class AnalyticsService {
     ...
     */
    async getMonthlyTotals(endMonth, count = 6) {
     ...
     */
    async compareMonths(currentMonth, previousMonth) {
     ...
     */
    async getTrendAnalysis(currentMonth, months = 6) {
        ...
        const changeVariances = monthlyChanges.map(item => Math.pow(item.change - averageMonthlyChange, 2));
        ...
        if (Math.abs(currentVsAveragePercent) > 15) {
            insights.push(`Current month spending is ${currentVsAveragePercent > 0 ? 'above' : 'below'} average by ${Math.abs(currentVsAveragePercent).toFixed(1)}%`);
        }
        // Volatility insights
        ...
        if (monthlyChanges.length > 0) {
            ...
            if (Math.abs(recentChange.percentageChange) > 20) {
                insights.push(`Significant ${recentChange.change > 0 ? 'increase' : 'decrease'} of ${Math.abs(recentChange.percentageChange).toFixed(1)}% from last month`);
            }
        }
        return {
            currentMonth: {
